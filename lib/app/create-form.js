...
/**
 * 读取字段定义（create 模式）
 * 支持 JSON 字符串或文件路径，返回 { fields, columns }
 */
function readFieldsDefinition(fieldsJsonOrFile) {
  ...
  // 支持两种格式：数组（字段列表）或对象（含 fields 和 columns）
  if (Array.isArray(parsed)) {
    return { fields: parsed, columns: 1 };
  }
  if (parsed && Array.isArray(parsed.fields)) {
    return { fields: parsed.fields, columns: parsed.columns || 1 };
  }

  console.error(t('create_form.fields_must_be_array'));
  process.exit(1);
}
