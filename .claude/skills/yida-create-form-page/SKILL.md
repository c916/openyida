...
| --- | --- | --- |
| `appType` | 是 | 应用 ID，如 `APP_E0MZ4VB75ZMB1BIGNVT4` |
| `formUuid` | 是 | 表单页面的唯一标识，如 `FORM-EFFAA9300A864658AB27009B8EBE805FFXOB` |
...
**formula 格式**：
```
SERIALNUMBER("corpId", "appType", "formUuid", "fieldId", "{\"type\":\"custom\",\"value\":[...]}")
```

其中最后一个参数是 JSON 字符串，包含 `serialNumberRule` 的完整配置。
...
1. 准备字段定义 JSON 文件
2. 读取项目根目录的 `.cookies.json` 获取登录态（包含 corpId）；若不存在则自动调用 `login.py` 触发扫码登录
...
7. **将返回的 `formUuid` 和各字段 `fieldId` 记录到 `RD/<项目名>.md` 的表单配置章节**

### update 模式

1. 读取项目根目录的 `.cookies.json` 获取登录态（包含 corpId）；若不存在则自动调用 `login.py` 触发扫码登录
...
5. **记录配置** → 将 `formUuid` 和字段 ID 写入 `RD/<项目名>.md`
