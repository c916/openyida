---
name: yida-publish
description: 宜搭自定义页面发布技能，将源码编译并部署 Schema 到宜搭平台。使用 Node.js 编写，集成 Babel 编译、Schema 构建和接口调用。
---
...
| `formUuid` | 自定义页面 ID | `FORM-520F9514C72A4A7DA0793ED67E5BC9DE83SL` |
| `源文件路径` | 源码文件路径（相对于项目根目录） | `src/index.js` |

> `baseUrl` 无需手动传入，脚本会自动从 `.cookies.json` 读取登录态（若不存在或接口返回 302，则自动触发扫码登录），并从中读取 `base_url`。

**示例**：

```bash
node publish.js APP_E0MZ4VB75ZMB1BIGNVT4 FORM-XXXXXX src/demo.js
```
...
3. **读取登录态**：读取项目根目录的 `.cookies.json`；若不存在则自动调用 `login.py` 触发扫码登录
...
> **注意**：发布目标地址由 `.cookies.json` 中保存的 `base_url` 决定（即登录后浏览器实际跳转到的域名），而非 `config.json` 中的 `loginUrl`。详见 `yida-login` 技能文档。
