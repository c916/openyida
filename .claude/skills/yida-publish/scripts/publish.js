 ...
 *
 * 示例：
 *   node publish.js APP_E0MZ4VB75ZMB1BIGNVT4 FORM-xxx src/index.js
 *
 * 流程：
 * 1. 读取源文件，通过 @ali/vu-babel-transform 编译 + UglifyJS 压缩
 * 2. 用代码动态构建 Schema，将 source/compiled 填入 actions.module
 * 3. 读取本地 .cookies.json 获取登录态；若未登录或接口返回 302，则调用 login.py 重新登录
...
const CONFIG = require("fs").existsSync(CONFIG_PATH) ? JSON.parse(require("fs").readFileSync(CONFIG_PATH, "utf-8")) : {};
...
const LOGIN_SCRIPT = path.resolve(
  SCRIPT_DIR,
  "..",
  "..",
  "yida-login",
...
function parseArgs() {
  ...
  if (args.length < 3) {
    ...
    console.error(
      "示例: node publish.js APP_xxx FORM-xxx src/index.js"
    );
    process.exit(1);
  }
  return {
...
function compileSource(sourcePath) {
  ...
  const compiledPath = path.join(parsedPath.dir, `${parsedPath.name}.compile.js`);
  const compiledFileName = path.basename(compiledPath);
...
const COOKIES_PATH = (() => {
  // 向上查找项目根目录（含 config.json 或 .git 的目录）
  let dir = SCRIPT_DIR;
  for (let i = 0; i < 10; i++) {
    if (
  ...
  return path.join(SCRIPT_DIR, "..", "..", "..", "..", ".cookies.json");
})();
...
async function main() {
  ...
  const compiledPath = path.join(parsedSource.dir, `${parsedSource.name}.compile.js`);
  ...
  // .cookies.json 中没有 csrf_token，必须通过 login.py 无头验证获取
  console.log("\n🔑 Step 2: 读取登录态");
