---
name: get-schema
description: 宜搭表单 Schema 获取技能，通过调用 getFormSchema 接口获取指定表单的完整 Schema 结构，用于分析字段定义、组件配置等。
---
...
```bash
node .claude/skills/get-schema/scripts/get-schema.js "APP_E0MZ4VB75ZMB1BIGNVT4" "FORM-xxx"
```
...
1. 读取项目根目录的 `.cookies.json` 获取登录态；若不存在则自动调用 `login.py` 触发扫码登录
