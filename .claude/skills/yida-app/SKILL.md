...
创建应用
    ↓
需求分析 → 创建 RD 文档（RD/<项目名>.md）
    ↓
创建自定义页面
    ↓
（如需存储数据）创建表单 → 将字段描述信息写入 RD 文档
    ↓
调用 yida 技能编写页面代码（src/<name>.js）
    ↓
自动调用 yida-publish 技能发布代码
```

> 💡 **登录态说明**：各脚本会自动读取项目根目录的 `.cookies.json`，无需手动执行登录命令。首次运行或 Cookie 失效时，脚本会自动打开浏览器引导扫码登录。
...
在执行创建页面前，**必须检查 RD 文档中的 corpId 与当前登录态的 corpId 是否一致**：

1. **读取 RD 文档中的应用配置**，获取已记录的 `corpId`
2. **读取 `.cookies.json`**，获取当前登录态的 `corpId`
3. **对比两个 corpId**：
   - **如果一致**：继续执行创建页面
   - **如果不一致或 RD 中无 corpId**：提示用户选择处理方式

**corpId 不一致时的处理选项**：

| 场景 | 建议操作 |
| --- | --- |
| RD 中有 corpId，但与当前登录态不一致 | **询问用户**：是重新登录到 RD 中的组织，还是在当前组织新建应用？ |
| RD 中无 corpId | 直接新建应用 |

**决策流程**：
```
检查 RD.corpId vs Cookie.corpId
    │
    ├── 一致 → 继续创建页面
    │
    └── 不一致
        ...
        │
        └── 用户选择"新建应用" → 回到 Step 1 创建新应用（会自动覆盖 RD 配置）
```
...
**输出**：`appType`（如 `APP_XXXXXX`），用 markdown 记录到 RD 文件夹下文档备用。
...
**输出**：`pageId`（如 `FORM-XXXXXX`），记录到 RD 文档备用。

> 详见 `yida-create-page` 技能文档。

---

### Step 3：需求分析 → 写入 RD 文档

如果是一句话生成应用，将分析的需求文档结果写入 `RD/<项目名>.md`：
...
> **重要**：
> - RD 文档只记录**业务语义信息**（应用名称、页面名称、页面类型、字段名称、字段类型、字段说明），**不记录** `formUuid`、`fieldId` 等 Schema 中的具体 ID 值
> - Schema ID（`formUuid`、`fieldId` 等）写入 `.cache/<项目名>-schema.json` 临时文件，供编码时读取，**注意不要写在系统的其他文件夹中**
> - **每次创建或修改表单/自定义页面后，必须同步更新 RD 文档中对应的页面/字段信息**
...
将字段定义写入 `RD/<项目名>-pageName-field.md`
...
#### 5.3 将表单信息写入 RD 文档和 .cache 临时文件

**写入 RD 文档**（只记录业务语义信息，不记录 Schema ID）：
...
调用 `yida` 技能，根据 RD 文档中的需求和配置信息编写自定义页面代码。
...
调用 `yida-publish` 技能，将源码编译并部署到宜搭平台。
...
```bash
node publish.js APP_E0MZ4VB75ZMB1BIGNVT4 FORM-XXXXXX src/index.js
```
...
2. UglifyJS 压缩
3. 动态构建 Schema JSON
4. 读取 `.cookies.json` 获取登录态（若不存在或接口返回 302，自动触发扫码登录）
5. 调用 `saveFormSchema` 接口保存 Schema

> 详见 `yida-publish` 技能文档。
...
| `get-schema` | 获取表单 Schema | 需要分析已有表单字段结构时 |
| `yida` | 编写自定义页面代码 | Step 6，核心开发阶段 |
| `yida-publish` | 编译并发布代码 | Step 7，部署到宜搭 |
...
| 信息类型 | 存储位置 | 示例 |
| --- | --- | --- |
| 应用名称、页面名称、页面类型、字段名称、字段类型、字段说明 | `RD/<项目名>.md` | 姓名、TextField / 单行文本、必填 |
