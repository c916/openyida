 ...
 *
 * 前置条件：
 *   项目根目录下需存在 .cookies.json（由 yida-login 生成）。
 ...
 *
 * 流程：
 * 1. 从 .cookies.json 读取登录态（cookies + base_url）
...
/**
 * 查找项目根目录下的 RD/<项目名>.md 文件
 * 通过向上查找包含 RD 文件夹的目录来确定项目根目录
 */
function findRdFile() {
  // 尝试从当前工作目录开始查找
  ...
  // 向上查找最多 5 层
  for (let i = 0; i < 5; i++) {
    const rdDir = path.join(currentDir, "RD");
...
`;
    ...
    console.error(`  ✅ 已更新 RD 文档: ${path.basename(rdFilePath)}`);
    console.error(`     - corpId: ${corpId}`);
    console.error(`     - appType: ${appType}`);
    console.error(`     - baseUrl: ${baseUrl}`);
    return true;
  } catch (err) {
    console.error(`  ⚠️  更新 RD 文档失败: ${err.message}`);
    return false;
  }
}
