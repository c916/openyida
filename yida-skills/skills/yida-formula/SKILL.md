...
- 不要在公式中使用不存在的函数，只能使用 `formula-functions.md` 中列出的函数
- 不要在公式中直接写字段名，必须使用字段 ID（如 `textField_xxx`）
- 不要将业务关联规则（INSERT/UPDATE/DELETE）配置在字段默认值公式中，必须在「业务关联规则」中配置
- 不要在校验规则公式中使用副作用函数（INSERT/UPDATE/DELETE）

## 严格要求 (MUST DO)

- 使用字段前必须先用 `yida-get-schema` 获取字段 ID，不得猜测
- 公式中引用其他字段必须使用完整字段 ID，不得使用字段标签名
- 配置业务关联规则前必须确认目标表单的 formUuid 和字段 ID

## 适用场景

| 用户意图 | 触发条件 |
|---------|---------|
| 字段自动计算 | "自动计算"、"默认值公式"、"根据字段计算" |
| 提交校验规则 | "校验"、"必须满足条件才能提交" |
| 业务关联规则 | "提交后自动新增/更新其他表单数据" |
